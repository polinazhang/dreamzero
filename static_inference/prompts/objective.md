...
For launching and the main body of code, you should write inside `/coc/testnvme/xzhang3205/vla-adaptation/models/dreamzero/static_inference`. Since the goals require some change in dreamzero architecture, you're allowed to modify ``/coc/testnvme/xzhang3205/vla-adaptation/models/dreamzero` under restrictions below:
...
DreamZero pads action chunks to max_action_dim=32 and applies action_mask elementwise to the action flow-matching MSE. When vision grad norm is calculated, use the original DreamZero action target. Your implementation should match the original dreamzero loss semantics exactly. **Do not invent a new loss.**
...
Results should be written to subfolders with timestamps inside `/coc/testnvme/xzhang3205/static/dreamzero`